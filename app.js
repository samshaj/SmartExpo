...
const PLANET_PRESETS = {
  "Earth": { luminosity: 1.0, distance: 1.0, radius: 1.0, mass: 1.0, eccentricity: 0.017, albedo: 0.30 },
  "Kepler-186f": { luminosity: 0.0412, distance: 0.432, radius: 1.17, mass: 1.71, eccentricity: 0.04, albedo: 0.30 },
  "Proxima Centauri b": { luminosity: 0.00155, distance: 0.0485, radius: 1.03, mass: 1.17, eccentricity: 0.11, albedo: 0.30 },
  "TRAPPIST-1e": { luminosity: 0.000553, distance: 0.0293, radius: 0.92, mass: 0.69, eccentricity: 0.008, albedo: 0.30 },
  "Kepler-22b": { luminosity: 0.79, distance: 0.849, radius: 2.38, mass: 9.1, eccentricity: 0.0, albedo: 0.30 },
  "HD 209458 b": { luminosity: 1.61, distance: 0.0475, radius: 15.5, mass: 219.0, eccentricity: 0.0, albedo: 0.10 },
  "Kepler-452b": { luminosity: 1.2, distance: 1.046, radius: 1.63, mass: 5.0, eccentricity: 0.035, albedo: 0.30 },
  "WASP-12b": { luminosity: 2.1, distance: 0.0229, radius: 21.3, mass: 467.0, eccentricity: 0.0, albedo: 0.06 },
  "Gliese 667 C c": { luminosity: 0.0137, distance: 0.125, radius: 1.54, mass: 3.8, eccentricity: 0.27, albedo: 0.30 },
  "TOI-700 d": { luminosity: 0.023, distance: 0.163, radius: 1.14, mass: 1.72, eccentricity: 0.03, albedo: 0.30 }
};
...
function updateUI() {
  ...
  // Temperature estimation: Temp_kelvin = 278.5 * ((energy * (1 - albedo))^0.25)
  const tempKelvin = 278.5 * Math.pow(energyReceived * (1 - albedo), 0.25);
...
function drawScene(timestamp) {
  ...
  // Scale: Auto-scale system size to fit orbital parameters
  // Max distance is 4.0 AU. We scale so that outerEdge / orbit size fits nicely.
  const maxSimDist = Math.max(d * (1 + e), outerEdge, 1.5);
  const baseScale = (Math.min(width, height) / 2) * 0.8;
  const scale = baseScale / maxSimDist; // Pixels per AU
  ...
  if (isRunning) {
    // Keplerian speed approximation: speed inversely proportional to distance^1.5
    const speed = 0.001 * elapsed * (1 / Math.pow(d, 1.5));
    orbitAngle += speed;
  }
